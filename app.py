...
def get_show(id):
    ...
    return create_response({"id" : db.getById('shows', int(id))})
...
def create_show():
    ...
    if ("episodes_seen" not in data):
        return create_response(status=422, message="Episodes seen not provided")
    data = db.create("shows", data)
    return create_response(data, status=201)
    #
    # return create_response({"id": db.getById('id', int(data['id']))}, status=201)
